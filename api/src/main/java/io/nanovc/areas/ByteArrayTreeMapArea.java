 ...
 * It is backed by a {@link TreeMapArea} which always keeps its contents sorted.
    ...
    implements ByteArrayAreaAPI
{
    ...
    /**
     * This either casts the given area to a {@link ByteArrayTreeMapArea} if it creates a new one with the same content as the given one.
     ...
     * @return The given area either casted to a {@link ByteArrayTreeMapArea} if it already is one or a new copy with the same content.
     */
    public static ByteArrayTreeMapArea castOrClone(AreaAPI<? extends ContentAPI> area)
    {
        if (area instanceof ByteArrayTreeMapArea) return (ByteArrayTreeMapArea) area;
        else
        {
